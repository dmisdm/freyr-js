...
const stripchar_1 = require("stripchar");
/**
...
function stripText(data) {
    return [
        ...new Set(data.reduce((all, text) => ((text = stripchar_1.StripChar.RSspecChar(text)) ? all.concat([text.replace(/\s{2,}/g, ' ').toLowerCase()]) : all), [])),
    ];
}
/**
...
function getWeight(a, b) {
    return (((b = b.join(' ').split(' ')), a.map(v => v.split(' ').every(p => b.includes(p))).filter(v => !!v).length / a.length) * 100);
}
exports.default = { stripText, getWeight };
