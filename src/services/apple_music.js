...
import { Client } from "@dmisdm/apple-music";
...
export default class AppleMusic {
  ...
  constructor(config) {
    ...
    if (typeof config !== "object")
      throw new Error(
        `[AppleMusic] Please define a configuration as an object`
      );
    if (!config.developerToken)
      throw new Error(
        `[AppleMusic] Please define [developerToken] as a property within the configuration`
      );
    this.#store.core = new Client({ developerToken: config.developerToken });
    let custom_head = { headers: { origin: "https://music.apple.com" } };
    this.#store.core.albums.axiosInstance =
      this.#store.core.albums.axiosInstance.create(custom_head);
    this.#store.core.artists.axiosInstance =
      this.#store.core.artists.axiosInstance.create(custom_head);
    this.#store.core.playlists.axiosInstance =
      this.#store.core.playlists.axiosInstance.create(custom_head);
    this.#store.core.songs.axiosInstance =
      this.#store.core.songs.axiosInstance.create(custom_head);
