...
import fileMgr from "./file_mgr.js";
import pFlatten from "./p_flatten.js";
import FreyrCore from "./freyr.js";
import AuthServer from "./cli_server.js";
import AsyncQueue from "./async_queue.js";
import parseRange from "./parse_range.js";
import StackLogger from "./stack_logger.js";
...
function prepProgressGen(options) {
  return (size, slots, opts, indentLen, isFragment) => {
    ...
    return ProgressBar.stream(size, slots, {
      ...
      label: "Downloading",
      variables: {
        _tag: `:{tag} (${isFragment ? "fragments" : "chunks"}: ${
          slots.length
        })`,
        bullet: "\u2022",
        bars: ({ total }) =>
          (Number.isFinite(total) && !forceFirst
            ...
            : [
                `:{indent}  | [:{bar}]${
                  Number.isFinite(total) ? " [:3{percentage}%]" : ""
                } [:{speed}] (:{eta}) [:{size}]`,
              ]
          ).join("\n"),
...
async function init(packageJson, queries, options) {
  ...
  try {
    ...
    Config.downloader.order = PROCESS_DOWNLOADER_ORDER(
      Config.downloader.order,
      (item) => {
        if (item)
          throw new Error(
            `Downloader order within the config file must be valid. found [${item}]`
          );
  ...
  if (
    (await processPromise(
      Promise.promisify(fs.access)(BASE_DIRECTORY, fs.constants.W_OK),
      stackLogger,
      {
  ...
  function createPlaylist(
    ...
    if (options.playlist !== false) {
      const validStats = stats.filter((stat) =>
        stat.code === 0 ? stat.complete : !stat.code
      );
      if (validStats.length) {
        ...
        const isNew =
          !(fs.existsSync(playlistFile) && fs.statSync(playlistFile).size) ||
  ...
  const downloadQueue = new AsyncQueue(
    ...
    async ({ track, meta, feedMeta, trackLogger }) => {
      ...
      const imageFile = await fileMgr({
        ...
        tempdir:
          Config.dirs.cacheDir === "<tmp>" ? undefined : Config.dirs.cacheDir,
          ...
          feedMeta.protocol !== "http_dash_segments"
            ? {
                ...
                opts: {
                  ...
                  failureMessage: (err) =>
                    trackLogger.getText(
                      `| [\u2715] Failed to get raw media stream${
                        err ? ` [${err.code || err.message}]` : ""
                      }`
                    ),
                },
            ...
            : {
                ...
                opts: {
                  failureMessage: (err) =>
                    trackLogger.getText(
                      `| [\u2715] Segment error while getting raw media${
                        err ? ` [${err.code || err.message}]` : ""
                      }`
                    ),
                },
    ...
    async ({ track, meta, files, audioSource }) => {
      return Promise.promisify(atomicParsley)(meta.outFilePath, {
        ...
        rDNSatom: [
          ...
          [
            `${audioSource.service[symbols.meta].DESC}: ${
        ...
        copyright: track.copyrights.sort(({ type }) =>
          type === "P" ? -1 : 1
        )[0].text, // cprt
  ...
  const postProcessor = new AsyncQueue(
    ...
    4,
    async ({ track, meta, files, audioSource }) => {
      await mkdirp(meta.outFileDir).catch((err) =>
        Promise.reject({ err, code: 6 })
      );
      const wroteImage =
        !!options.cover &&
        ((outArtPath) =>
      ...
      if ((result.service = iterator.next().value)) {
        result.sources = Promise.resolve(
          ...
          if ([undefined, null].includes(source))
            throw new Error(
              `incompatible response item. recieved: [${source}]`
            );
          if (!("getFeeds" in source))
            throw new Error(
              `service provided no means for source to collect feeds`
            );
          let feedTries = 1;
          const getFeeds = () =>
            source
  ...
  const trackQueue = new AsyncQueue(
    ...
    async ({ track, meta, props }) => {
      ...
      const feedMeta = audioFeeds.formats.sort((meta1, meta2) =>
        meta1.abr > meta2.abr ? -1 : meta1.abr < meta2.abr ? 1 : 0
      )[0];
  ...
  async function trackHandler(query, { service, queryLogger }) {
    ...
    logger.log(
      `\u27a4 Playtime: ${
        TimeFormat.fromMs(track.duration, "mm:ss").match(
          /(\d{2}:\d{2})(.+)?/
        )[1]
      }`
    );
  ...
  async function artistHandler(query, { service, queryLogger }) {
    ...
    if (artist.followers)
      logger.log(
        `\u27a4 Followers: ${`${artist.followers}`.replace(
          /(\d)(?=(\d{3})+$)/g,
          "$1,"
        )}`
      );
    ...
    return Promise.mapSeries(albumsStack, async ({ uri }, index) => {
      ...
      const albumLogger = collationLogger
        .log(
          `(${prePadNum(index + 1, albumsStack.length)}) [${album.name}] (${
            album.type
          })`
        )
        .tick();
  ...
  async function playlistHandler(query, { service, queryLogger }) {
    ...
    if (playlist.description)
      logger.log(`\u27a4 Description: ${entityDecode(playlist.description)}`);
    logger.log(`\u27a4 Type: ${playlist.type}`);
    if (playlist.followers)
      logger.log(
        `\u27a4 Followers: ${`${playlist.followers}`.replace(
          /(\d)(?=(\d{3})+$)/g,
          "$1,"
        )}`
      );
    ...
    async (query) => {
      ...
      let queryStats = await pFlatten(
        (contentType === "track"
          ...
          .catch((err) =>
            queryLogger.error(
              `\x1b[31m[i]\x1b[0m An error occurred while processing the query${
                err ? ` (${err.message || err})` : ""
              }`
            )
          )
        ...
        async (queryStat) => {
          ...
          await Promise.mapSeries(trackStats, async (trackStat) => {
            ...
            if (trackStat.code) {
              ...
              embedLogger.error(
                `\u2022 [\u2715] ${
                  trackStat.meta && trackStat.meta.trackName
          ...
          if (queryStat.isCollection)
            createPlaylist(
              ...
              `Playlist: ${source.name}${
                source.owner_name ? ` by ${source.owner_name}` : ""
              }`,
  ...
  if (options.stats) {
    ...
    stackLogger.log(
      ` [\u2022] Cover Art: ${options.cover} (${Config.image.height}x${Config.image.width})`
    );
    stackLogger.log(
